#!/usr/bin/env python3
"""Preflight for PR38 usertable authSubjectId.
...
def main() -> int:
    ...
    with engine.connect() as conn:
        ...
        meta = _column_meta(conn, "authSubjectId")
        if meta is None:
            print("INFO: column absent (will be added): authSubjectId")
        else:
            print(f"INFO: column already present: authSubjectId → {meta}")

        # Duplicate non-null authSubjectId would block unique constraint.
        try:
            if meta is not None:
                dups = conn.execute(
                    text(
                        "SELECT authSubjectId, COUNT(*) AS c "
                        "FROM usertable "
                        "WHERE authSubjectId IS NOT NULL "
                        "AND authSubjectId != '' "
                        "GROUP BY authSubjectId "
                        "HAVING c > 1 "
                        "LIMIT 20"
                    )
                ).fetchall()
                if dups:
                    blocking = True
                    print(
                        "BLOCKING: duplicate authSubjectId values "
                        f"(showing up to 20): {dups}"
                    )
                else:
                    print("OK: no duplicate non-empty authSubjectId values.")
        except Exception as exc:
