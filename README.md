...
**PR45 (2026-08-13):** `request.cancelled` known-party + **CXL1** marketplace stale-removal **IMPLEMENTED** (outbox emission on `DELETE /deleterequest` when master ∧ `DOMAIN_EVENT_REQUEST_CANCELLED_ENABLED`). Flag default **false**. Immediate marketplace rebuilds = **0**; customer + existing bidders get known-party refresh; never-bid vendors reconcile via `*/30` / reconnect (≈30–60s). **Not deployed / not Azure-enabled**. M5 still pending — do not enable. Process-bound expected: `requestCancelled.perEventEnabled=false`, `requestCancelled.emissionEnabled=false`.

**PR46 (2026-08-13):** `request.reopened` Connected-First marketplace runtime **IMPLEMENTED** (outbox emission on `PUT /reopenbooking` when master ∧ `DOMAIN_EVENT_REQUEST_REOPENED_ENABLED`). Flag default **false**. Single event per reopen txn; aggregateId/payload.requestId = **NEW RID**; zero `request.created` duplicate; Customer known-party + Connected-First marketplace (cap 25). **Not deployed / not Azure-enabled**. Do not deploy before `request.created` canary PASS. Process-bound expected: `requestReopened.perEventEnabled=false`, `requestReopened.emissionEnabled=false`.
