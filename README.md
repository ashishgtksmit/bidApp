...
**PR40:** Known-party lifecycle events on the same outbox pipeline: `bid.updated`, `bid.deleted`, `bid.accepted`, `handshake.cancelled`, `handshake.accepted`, `handshake.rejected`, `booking.cancelled_by_customer`, `driver.assignment_changed`. Emission requires master `DOMAIN_EVENTS_ENABLED` **and** the matching `DOMAIN_EVENT_*_ENABLED` flag (code defaults **false**). Recipients are known parties only (no marketplace fan-out). FCM remains FastAPI post-commit for mutations that already send FCM (`bid.updated` intentionally sends **none**). **No new DB migration**. Production Azure Redis remains private-only. **Production Waves A1–A3 + B2 (2026-08-08) + B3 (2026-08-09):** `bid.updated` / `bid.deleted` / `bid.accepted` / `handshake.cancelled` / `handshake.accepted` = true on canary-proven path (B3 rev `da72741`); independent soaks in progress (not accepted). **Wave B4 canary (2026-08-10):** PASS WITH NON-BLOCKING CAVEAT on observability rev `4601b3f` (dedicated `handshakeRejected` process-bound block); `DOMAIN_EVENT_HANDSHAKE_REJECTED_ENABLED=true`; soak in progress (not accepted). Remaining booking / driver unset/false. See `docs/OPENBID_KNOWN_PARTY_REALTIME_EVENTS_PR40_PLAN.md`.

**PR41 (2026-08-07, Partially Executed; Wave A1–B3 soaks + B4 readiness 2026-08-09):** Domain-event flag control hardened in deploy **`5939805`**: `load_dotenv(override=False)`, fail-closed `env_flag_enabled`, startup `domain_event_flag_snapshot`, emission decision logs, `tests/test_pr41_domain_event_flag_control.py`. Production matrix A–D passed (master × `bid.created`); poller fallback server-side passed when `DOMAIN_EVENT_BID_CREATED_ENABLED=false`. Deploy **`b915cd5`** preserves `fcmToken` when login omits it. Pass 5: two-Flutter L2 PASS; E9 Android FCM PASS; E11 iOS still open. **Waves A1/A2/A3** enabled on `b915cd5` with independent soaks (not accepted). **Wave B2** fixed/redeployed as **`06f9579`** (process-bound `GET /domain-event-flag-snapshot`, stdout event logging, cancel-path decision markers); retry PASS; soak in progress. **Wave B3** observability deploy **`da72741`** adds dedicated `handshakeAccepted` block; controlled canary **PASS WITH NON-BLOCKING CAVEAT**; `DOMAIN_EVENT_HANDSHAKE_ACCEPTED_ENABLED=true`; soak in progress. **Wave B4** controlled canary PASS WITH NON-BLOCKING CAVEAT on rev `4601b3f`; flag true; soak in progress; C1 unset. See `docs/OPENBID_EVENT_PIPELINE_PRODUCTION_VALIDATION_PR41_PLAN.md`.
