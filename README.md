...
**PR40:** Known-party lifecycle events on the same outbox pipeline: `bid.updated`, `bid.deleted`, `bid.accepted`, `handshake.cancelled`, `handshake.accepted`, `handshake.rejected`, `booking.cancelled_by_customer`, `driver.assignment_changed`. Emission requires master `DOMAIN_EVENTS_ENABLED` **and** the matching `DOMAIN_EVENT_*_ENABLED` flag (all default **false**). Recipients are known parties only (no marketplace fan-out). FCM remains FastAPI post-commit. **No new DB migration** — production `openbid_domain_outbox` audited 2026-08-06 (`eventType varchar(64)`, JSON payload). Production Azure Redis Streams probed 2026-08-06 for PR40 envelopes (isolated probe stream; production stream untouched; network restored private-only). Production PR40 flags **not enabled**. See `docs/OPENBID_KNOWN_PARTY_REALTIME_EVENTS_PR40_PLAN.md`.

**PR41 (2026-08-06, Partially Executed; Completion Pass 2):** Production inventory re-verified on `Openbid-API`: `DOMAIN_EVENTS_ENABLED=true`, `DOMAIN_EVENT_BID_CREATED_ENABLED=true`, all PR40 flags **unset/false**. Active deploy **`99ffd5d`** includes PR39 `bid.created` outbox append; live outbox counts still **0/0/0** (no post-deploy canary mutation). No PR40 production flag enabled. Production canary / device QA / staging load **not** executed — stopped before mutation (credentials/devices unavailable). See `docs/OPENBID_EVENT_PIPELINE_PRODUCTION_VALIDATION_PR41_PLAN.md`.
